...
contract Config {
    ...
    uint256 constant TREETIMES = 12500 * 1e18;
...
contract InvestorRelationship is Config, SeroInterface {
    string constant public INVEST_CURRENCY = "HAPY";
    ...
    constructor() public {
        investors.push(Investor({refereeId : 0, largeAreaId : 0, amount : 0, returnAmount : 0, achievement : 0, otherAchievement : 0, recommendNum : 0, profitLevel : 0, addr : 0, star : 0, isKing : false, value : 0}));
    ...
    function findAddrById(uint256 id) public view returns (address) {
        return findById(id).addr;
    }

    function balanceOfHAPY() public view returns (uint256) {
        return sero_balanceOf(INVEST_CURRENCY).sub(cash);
    }
    ...
    function insert(uint256 refereeId, uint256 amount, address addr) internal {
        ...
        investors.push(Investor({refereeId : refereeId, largeAreaId : 0, amount : amount, returnAmount : 0, achievement : 0, otherAchievement : 0, recommendNum : 0, profitLevel : 0, addr : addr, star : 0, isKing : false, value : 0}));
        returnRewards.push(ReturnReward({staticReward : 0, recommendReward : 0, starReward : 0, vipReward : 0, currentStaticReward : 0, currentIncome : 0, staticTimestamp : now, updateTimestamp : 0}));
    ...
    function _update(uint256 id, uint256 amount) internal {
        ...
        while (currentId != uint256(0)) {
            ...
            if (investors[currentId].largeAreaId == 0) {
                ...
                uint256 largeAchievement = investors[largeAreaId].achievement.add(investors[largeAreaId].amount);
                uint256 childAchievement = investors[childId].achievement.add(investors[childId].amount);
        ...
        if (height == investors[id].profitLevel) {
            ...
            investors[id].profitLevel = height;
        }
    }
    ...
    function _beforeUpdate() internal {
        if (!Utils.sameDay(now, lastUpdated)) {
            preRewardAmount = sero_balanceOf(INVEST_CURRENCY).sub(cash).div(30);
...
contract HapyGame is InvestorRelationship, Ownable {
    ...
    constructor(address _marketAddr, address _codeServiceAddr) public payable {
        marketAddr = _marketAddr;
        codeService = CodeService(_codeServiceAddr);
    }

    function balanceOf(string memory tokenName) public view returns (uint256) {
        return sero_balanceOf(tokenName);
    }
    ...
    returns (string, string, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint8, bool)  {
        ...
        if (i.largeAreaId != 0) {
            largeAchievement = investors[i.largeAreaId].achievement.add(investors[i.largeAreaId].amount);
        }
    ...
    function withdrawBalance() public {
        ...
        if (value > 0) {
            require(sero_send_token(msg.sender, INVEST_CURRENCY, value));
        }
    }
